SELECT CODPARC, NOMEPARC,  SUM(VALORANT) AS VALORANT, SUM(VALORAT) AS VALORAT ,
    ...
    SUM ((((ITE.VLRTOT)
        ...
        * CASE WHEN CAB.TIPMOV = 'D' THEN -1 ELSE 1 END) * VCAB.INDITENS) AS VALORANT, 
    0 AS VALORAT, PAR.NOMEPARC, PAR.CODPARC
    FROM TGFCAB CAB
    , TGFPAR PAR
    , TGFITE ITE
    , TGFPRO PRO
    ...
    AND ((CAB.CODPROJ= :CODPROJ) OR (:CODPROJ IS NULL))
...
SELECT
    ...
    SUM ((((ITE.VLRTOT)
        ...
        + (CASE WHEN :ST = 'S' THEN ITE.VLRSUBST ELSE 0 END)
        + (CASE WHEN :IPI = 'S' THEN ITE.VLRIPI ELSE 0 END))
        * CASE WHEN CAB.TIPMOV = 'D' THEN -1 ELSE 1 END) * VCAB.INDITENS) AS VALORAT,
